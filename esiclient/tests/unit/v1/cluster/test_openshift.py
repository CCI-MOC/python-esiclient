...
class TestWaitForNodes(TestCase):
    ...
    def test_wait_for_nodes(self, mock_caia, mock_sleep):
        first_response = []
        second_response = [{"status": "pending",
                            "requested_hostname": "host1"}]
        third_response = [{"status": "known", "requested_hostname": "host1"},
                          {"status": "known", "requested_hostname": "host2"}]
        fourth_response = [{"status": "known", "requested_hostname": "host1"},
                           {"status": "known", "requested_hostname": "host2"},
                           {"status": "pending",
                            "requested_hostname": "host3"}]
        fifth_response = [{"status": "known", "requested_hostname": "host1"},
                          {"status": "known", "requested_hostname": "host2"},
                          {"status": "known", "requested_hostname": "host3"}]

        mock_caia.side_effect = [MockResponse(first_response),
...
class TestOrchestrate(base.TestCommand):
    ...
    def setUp(self):
        ...
        def mock_find_subnet(name):
            ...
            return None
        self.app.client_manager.network.find_subnet.\
            side_effect = mock_find_subnet
    ...
    @mock.patch(
        ...
        autospec=True)
    @mock.patch(
        'esiclient.utils.get_or_create_port_by_ip',
        autospec=True)
    @mock.patch(
    ...
    def test_take_action(self, mock_load, mock_loads, mock_sleep, mock_caia,
        ...
        mock_caia.assert_has_calls([
            ...
            # no hosts ready
            call("infra-envs/%s/hosts" % self.infra_env_id,
                 'get', headers),
            # get boot iso image url
            call(
        ...
        mock_gocp.assert_has_calls([
            ...
            call('esi-node2-private_network', self.private_network,
                 self.app.client_manager.network),
            call('esi-node3-private_network', self.private_network,
                 self.app.client_manager.network),
        ])
        assert mock_gocpbi.call_count == 2
        mock_gocpbi.assert_has_calls([
            call('1.1.1.1', 'test_cluster-api', self.private_network,
                 self.private_subnet, self.app.client_manager.network),
            call('2.2.2.2', 'test_cluster-apps', self.private_network,
                 self.private_subnet, self.app.client_manager.network)
        ])
        assert mock_goapfi.call_count == 2
        mock_goapfi.assert_has_calls([
            call(self.api_port, self.external_network,
                 self.app.client_manager.network),
            call(self.apps_port, self.external_network,
                 self.app.client_manager.network),
        ])
